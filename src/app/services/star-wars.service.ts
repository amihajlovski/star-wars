...
export class StarWarsService {
  ...
  public getPeople(page: number = 1, search: string): Observable<IStarWarsResponse> {
    return this.http
    ...
    .pipe(
      mergeMap((people: IStarWarsResponse) =>
        from(people.results).pipe(
          mergeMap(
            (character: IStarWarsCharacter) => this.http.get(character.homeworld),
            (character: IStarWarsCharacter, planet: IStarWarsPlanet) => ({ ...character, planet })
          ),
          toArray(),
  ...
  public getCharacter(id: string): Observable<IStarWarsCharacter> {
    return this.http
      .get<IStarWarsCharacter>(`${this.url}/people/${id}`)
      .pipe(
        mergeMap((result: IStarWarsCharacter) =>
          from([result]).pipe(
            ...
            mergeMap(
              character => {
                let allResidents = character.planet.residents.map((url: string) => this.http.get(url));
